    ...
    if email == 'admin@gmail.com' and password == 'abc':
        with pool.connect() as db_conn:
            ...
            if not exists:
                ...
                db_conn.execute(insert_user,
                                parameters={"user_id": user_num, "username": "admin", "email": "admin@gmail.com"})
                db_conn.commit()
                db_conn.close()
        ...
        with pool.connect() as db_conn:
            ...
            if not exists:
                ...
                db_conn.execute(insert_user,
                                parameters={"user_id": user_num, "username": "user", "email": "a@gmail.com"})
                db_conn.commit()
                db_conn.close()
...
def append_dict():
    ...
    info, source = multi_turn_search_sample(project_id=project_id, location=location, data_store_id=get_val(),
                                            search_queries=search_queries)  # type: ignore
    ...
    with pool.connect() as db_conn:
        ...
        db_conn.execute(insert_stmt,
                        parameters={"conversation_id": conversation_id, "user_id": user_num, "user_num": user_num,
                                    "timestamp": store_time, "message": payload["message"], "response": current_res,
                                    "datastore": active_store})
        db_conn.commit()
        db_conn.close()
